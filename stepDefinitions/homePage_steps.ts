...
When (/^I should be able to search with (.*)$/,function (criteria:any) {
  homePage.search(criteria);
});

When (/^I should successfully see user (.*) in serach results$/,function (criteria:any) {
  I.see(criteria);
  I.seeNumberOfElements('//tr[@class=\'smart-table-data-row ng-scope\']', 1);
});

Then (/^I should be able to delete user$/,function (dataTable:any) {
